...
# Set a time for the iterations (between 5 and 20 minutes, but allow for at least 5 minutes for pairs to discuss the solution before the iteration starts). If you do 5+15 minute iterations, you need approximately 3.5 hours (and the retrospective could take a while)
# Pair rotation rules: no developer stays on the same codebase for more than 2 iterations. Can be taken out, if the audience isn't familiar with pair programming, as this is not the main focus of the workshop
# Prepare people for failure. There will be a lot of it, and this exercise can bruise egos - make sure everyone understands that failure is where we're trying to get, so we can learn from it in a safe environment
# Begin!
